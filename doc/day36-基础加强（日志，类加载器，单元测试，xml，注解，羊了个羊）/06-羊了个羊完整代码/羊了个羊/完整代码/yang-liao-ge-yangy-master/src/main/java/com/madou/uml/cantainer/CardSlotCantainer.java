package com.madou.uml.cantainer;
...
public class CardSlotCantainer extends JPanel {
    ...
    static Clip failClip;
    static {
        try {
            URL winSound = ReadResourceUtil.readAudio("win.wav");
            AudioInputStream audioInputStream = AudioSystem.getAudioInputStream(new BufferedInputStream(winSound.openStream()));
        ...
        try {
            URL failSound = ReadResourceUtil.readAudio("fail.wav");
            AudioInputStream audioInputStream = AudioSystem.getAudioInputStream(new BufferedInputStream(failSound.openStream()));
            failClip = AudioSystem.getClip();
            failClip.open(audioInputStream);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
